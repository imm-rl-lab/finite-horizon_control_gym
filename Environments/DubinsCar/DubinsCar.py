...
class DubinsCar:
    def __init__(self, initial_state=np.array([0, 0, 0, 0]), dt=0.1, terminal_time=2 * np.pi, inner_step_n=10,
        ...
        self.action_space = spaces.Box(low=action_min, 
                                       high=action_max, shape=(1,), 
                                       dtype=np.float32)
        self.observation_space = spaces.Box(low=np.array([0,-100,-100,-100]), 
                                            high=np.array([terminal_time,100,100,100]), 
                                            dtype=np.float32)
        self.metadata = {"render_modes": ["human", "rgb_array", "single_rgb_array"],"render_fps": 30,}
        self.total_rewards = []
        self.total_reward = 0
