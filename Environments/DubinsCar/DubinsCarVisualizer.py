...
class DubinsCarVisualizer:
    ...
    def show_fig(self, env, agent, sessions):
        ...
        plt.subplot(231)
        plt.plot(4,0,'ro', label='target state')
        ...
        plt.subplot(232)
        plt.plot(env.terminal_time,0.75 * np.pi,'ro', label='target state')
        plt.plot(states[-1][0],states[-1][3],'bo', label='terminal state')
        plt.plot([state[0] for state in states],[state[3] for state in states],'m--', label='angel')
        plt.xlim((0, env.terminal_time+0.5))
        ...
        plt.subplot(235)
        plt.plot(self.noise_thresholds,'g', label='noise thrasholds')
        plt.legend()
        plt.grid()
