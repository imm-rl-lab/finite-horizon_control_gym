...
class TargetProblemVisualizer:
    ...
    def show_fig(self, env, agent, sessions):
        ...
        plt.subplot(231)
        plt.plot([state[3] for state in states], [state[4] for state in states], 'm--', label='trajectory')
        plt.plot(env.target_point[0],env.target_point[1],'ro', label='target state')
        plt.plot(states[-1][3],states[-1][4],'bo', label='terminal state')
        plt.xlim((-3, 3))
        plt.ylim((-3, 3))
        plt.grid()
        ...
        plt.plot(0,0,'ro', label='target state')
        ...
        plt.subplot(233)
        plt.plot([action[0] for action in actions], [action[1] for action in actions], 'm--', label='actions')
        plt.xlim((-1.1, 1.1))
        plt.ylim((-1.1, 1.1))
        plt.grid()
